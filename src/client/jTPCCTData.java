...
public class jTPCCTData {
    ...
    public void generateNewOrder(Logger log, jTPCCRandom rnd, long due) {
        ...
        {
            newOrder.ol_i_id[i] = rnd.getItemIdByWarehouse(terminalWarehouse);
            if (rnd.nextInt(1, 100) <= 99)
                newOrder.ol_supply_w_id[i] = terminalWarehouse;
            else
                newOrder.ol_supply_w_id[i] = ShardingNumber.oneSharding(terminalWarehouse, rnd.nextInt(1, numWarehouses));
        ...
        {
            newOrder.ol_i_id[i - 1] = ShardingNumber.oneSharding(terminalWarehouse, rnd.nextInt(1, 9) * 1000000 + 
			    newOrder.ol_i_id[i - 1]);
            transRbk = true;
        }
	...
	*/
    ...
    public void generatePayment(Logger log, jTPCCRandom rnd, long due) {
        ...
        if (rnd.nextInt(1, 100) > 85) {
            ...
            while (payment.c_w_id == payment.w_id && (numWarehouses / PropertiesFactory.instance.shardingNumber) > 1)
                payment.c_w_id = ShardingNumber.oneSharding(payment.w_id, rnd.nextInt(1, numWarehouses));
        }
    ...
    private void executeOrderStatus(Logger log, jTPCCConnection db)
        ...
        try {
            ...
            // select the last ORDER max oid first
            stmt = db.stmtOrderStatusSelectMaxOid;
            stmt.setInt(1, orderStatus.w_id);
            stmt.setInt(2, orderStatus.d_id);
            stmt.setInt(3, orderStatus.c_id);
            rs = stmt.executeQuery();
            if (!rs.next()) {
                throw new Exception(String.format("Last Order find max(oid) failed!"));
            }
            int maxOrderOid = rs.getInt(1);
            rs.close();

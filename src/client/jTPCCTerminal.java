...
public class jTPCCTerminal implements jTPCCConfig, Runnable {
    ...
    public void run() {
        PerfermanceFactory.instance.threadInit(Thread.currentThread());
    ...
    private void executeTransactions(int numTransactions) {
        ...
        for (int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++) {
            ...
            if (!terminalWarehouseFixed) {
                terminalWarehouseID = PropertiesFactory.instance.wareHouseRangeConvert(rnd.nextInt(1,
                        PropertiesFactory.instance.getWarehouseRange()));
